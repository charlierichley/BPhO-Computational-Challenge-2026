...
method = intefrom c3_ext_functions import *
    ...
    def update(frame):
        ...
        # Not on initial frame - must update old curves
            ...
            for index, lst in enumerate(active_intervals):
                ...
                f_a = B(lamda_a * 1e-9, T, is_adap_integrate=True)
                f_b = B(lamda_b * 1e-9, T, is_adap_integrate=True)

                whole = simpson_basic(B, T, lamda_a * 1e-9, f_a, lamda_b * 1e-9, f_b)

                lamda_mid = (lamda_a + lamda_b) / 2
                f_mid = B(lamda_mid * 1e-9, T, is_adap_integrate=True)
